...
from read_raw import imgData
t = np.array(range(0,520))
y = np.array(imgData[355,:,275])-np.array(imgData[0,:,0])
#负值变零
for i in range(520):
    if y[i]>imgData[355,:,275][i]:
        y[i]=0

ysg = savitzky_golay(y, window_size=31, order=4)
import matplotlib.pyplot as plt
plt.plot(t, y, label='Noisy signal')
plt.plot(t, np.array(imgData[355,:,275]), 'k', lw=1.5, label='Original signal')
plt.plot(t, ysg, 'r', label='Filtered signal')
plt.legend()
plt.show()
